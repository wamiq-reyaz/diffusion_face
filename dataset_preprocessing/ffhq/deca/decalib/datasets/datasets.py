...
class TestData(Dataset):
    def __init__(self, testpath, iscrop=True, crop_size=224, scale=1.25, face_detector='fan', sample_step=10,
        ...
        if isinstance(testpath, list):
            ...
            # self.imagepath_list = [os.path.relpath(os.path.join(root, fname)) for root, _dirs, files in os.walk(testpath) for fname in files if os.path.splitext(fname)[1].lower() in PIL.Image.EXTENSION]
