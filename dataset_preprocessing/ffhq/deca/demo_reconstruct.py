...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        if os.path.isfile(os.path.join(savefolder, name, f'{name}.png')):
                continue
        if args.load_eye_pose:
            eye_pose = np.load(f'{args.eyepath}/{name}.npy')[-4:]
            eye_pose = torch.tensor([eye_pose[0], -eye_pose[1], 0, eye_pose[2], -eye_pose[3], 0]).cuda().view(1, -1)
        else:
            eye_pose = None
        images = testdata[i]['image'].to(device)[None,...]
        with torch.no_grad():
            codedict = deca.encode(images)
            opdict, visdict = deca.decode(codedict, eye_pose=eye_pose) #tensor
            if args.render_orig:
                tform = testdata[i]['tform'][None, ...]
                tform = torch.inverse(tform).transpose(1,2).to(device)
                original_image = testdata[i]['original_image'][None, ...].to(device)
                _, orig_visdict = deca.decode(codedict, render_orig=True, original_image=original_image, tform=tform, eye_pose=eye_pose)    
                orig_visdict['inputs'] = original_image            

        if args.saveDepth or args.saveKpt or args.saveObj or args.saveMat or args.saveImages or args.saveCoeff:
            os.makedirs(os.path.join(savefolder, name), exist_ok=True)
        # -- save results
        if args.saveCoeff:
            codes = {k: v.detach().cpu().numpy() for k, v in codedict.items()}
            if args.load_eye_pose:
                codes['eye_pose'] = eye_pose.detach().cpu().numpy()
            np.save(os.path.join(savefolder, name, name + '.npy'), codes)
        if args.saveDepth:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eyepath', default='TestSamples/examples/results', type=str,
                        help='path to the output directory, where results(obj, txt files) will be stored.')
    ...
    parser.add_argument('--saveCoeff', default=False, type=lambda x: x.lower() in ['true', '1'],
