...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._path):
            ...
            # DANGER: Tailored for FFHQ+jpg
            for ii in range(70000):
                full_path = os.path.join(self._path, str(ii).zfill(5) + '.jpg')
                if os.path.exists(full_path):
                    self._all_fnames.append(os.path.join(prefix, str(ii).zfill(5) + '.jpg'))
        ...
        # The remove the suffix to get image index. 
        idxes = [os.path.basename(fname.replace('.jpg', '')) for fname in self._image_fnames]
        valid_idxes = []
        for idx in idxes:
            if os.path.exists(os.path.join(self._mesh_path, idx, idx + '.obj')):
                valid_idxes.append(idx)

        self._mesh_fnames = [os.path.join(fname, fname + '.obj') for fname in valid_idxes]

        #fix the images as well
        self._image_fnames = [fname + '.jpg' for fname in valid_idxes]
    ...
    def _load_raw_lms(self, raw_idx):
        fname = self._mesh_fnames[raw_idx]
        lms = np.loadtxt(os.path.join(self._mesh_path, fname).replace('.obj', '_kpt2d.txt'))
